...
public class EditTeacherActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tb = findViewById(R.id.tbTeacherSettings);
        ...
        iv_pic = findViewById(R.id.btnChooseImageTeacher);
    ...
    private void setProfilePic() {
        stref.child("Profile Pics/"+org_id+"/Teachers/"+id).getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
            @Override
    ...
    private void uploadPicture(){
        stref.child("Profile Pics/"+org_id+"/Teachers/"+id).putFile(image_uri).addOnCompleteListener(new OnCompleteListener<UploadTask.TaskSnapshot>() {
            @Override
            public void onComplete(@NonNull Task<UploadTask.TaskSnapshot> task) {
                if(task.isSuccessful()){
                    Toast.makeText(EditTeacherActivity.this, "Profile updated", Toast.LENGTH_SHORT).show();
    ...
    public void btnClicked(View view) {
        ...
        if(new_email.trim().equals("") && new_pass.trim().equals("")){
            Toast.makeText(EditTeacherActivity.this, "Enter credentials", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        if(!new_email.equals(email)){
            ...
            ref.child(org_id).child("Teachers").child(id).child("email").setValue(new_email);

            FirebaseAuth.getInstance().getCurrentUser().updateEmail(new_email).addOnSuccessListener(new OnSuccessListener<Void>() {
                @Override
                public void onSuccess(Void aVoid) {
                    Toast.makeText(EditTeacherActivity.this, "Email updated", Toast.LENGTH_SHORT).show();
                }
            });
        }
        ...
        if(!new_pass.trim().equals("")){
            FirebaseAuth.getInstance().getCurrentUser().updatePassword(new_pass).addOnSuccessListener(new OnSuccessListener<Void>() {
                @Override
                public void onSuccess(Void aVoid) {
                    Toast.makeText(EditTeacherActivity.this, "Password updated", Toast.LENGTH_SHORT).show();
                }
            });
        }
    }
